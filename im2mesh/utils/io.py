...
def export_pointcloud(vertices, colors, out_file, as_text=True):
    ...
    # color_vector_dtype = [('c', 'f4')]
    # colors = colors.view(dtype=color_vector_dtype).flatten()
    plyel1 = PlyElement.describe(vertices, 'vertex')
    # plyel2 = PlyElement.describe(colors, 'color')
    # plydata = PlyData([plyel1, plyel2], text=as_text)
    plydata = PlyData([plyel1], text=as_text)
