...
class OccupancyNetwork(nn.Module):
    ...
    def compute_elbo(self, p, sdf, inputs, **kwargs):
        ''' Computes the expectation lower bound.

        Args:
            p (tensor): sampled points
            sdf (tensor): sdf values for p
            inputs (tensor): conditioning input
        '''
        # i have never tested it with a latent encoder!

        c = self.encode_inputs(inputs)
        q_z = self.infer_z(p, sdf, c, **kwargs)
        z = q_z.rsample()
        logits = self.decode(p, z, c, **kwargs)

        # replace BCE with L1 distance
        # rec_error = -p_r.log_prob(sdf).sum(dim=-1)
        rec_error = nn.functional.l1_loss(logits, sdf, reduction='none').sum(dim=-1)
        # does this also work? -no! returns empty shape
        # rec_error2 = nn.functional.l1_loss(logits, sdf, reduction='sum')
