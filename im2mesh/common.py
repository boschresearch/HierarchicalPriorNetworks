...
def get_crop_mask(pcd_p, roi_min, roi_max):
    """ determine mask of points within min and max
    Args:
        pcd_p (np.ndarray): array with points of shape Nx3
        roi_min (np.ndarray): x_min, y_min, z_min
        roi_max: same as above
    return:
        mask (np.ndarray): (N) whether point is within bounds or not
    >>> get_crop_mask(np.asarray([[1,1,1],[3,3,3]]), np.asarray([1,1,1]), np.asarray([2,2,2]))
    array([ True, False])
    """
    in_mask = np.all((roi_min <= pcd_p) & (pcd_p < roi_max), axis=1)
    return in_mask
...
def compute_iou(occ1, occ2):
    ...
    # creates nan if union of prediction and gt is 0
    # typically when part is empty
