...
parser.add_argument('config_patch', type=str, help='Path to patch config file.')
...
patch_setting = Settings(args.config_patch, device, part=True)
Settings.assert_compatible([bl_setting, patch_setting])

# generation directory
ioh = IOHelper(patch_setting, "_hierarchical")

# Count how many models already created
model_counter = defaultdict(int) # make this the full image model counter (batch)

for it, (data_bl, data_patch) in enumerate(
                                tqdm(zip(
    ...
    # TODO: make these methods a part of settings or move it to im2mesh/onet/generation.py
    # TODO or keep as is in order to keep all part-functions in one place
    vg_global, _ = get_valuegrid_baseline(data_bl, bl_setting)
    vg_local, _ = get_valuegrid_patch(data_patch, patch_setting)
    combined = (vg_local + vg_global) / 2
